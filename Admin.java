...
public class Admin extends User {
    ...
    public void seeMemberHist(Member[] member, int memberMax)
    {
        ...
        for (int i = 0; i<memberMax; i++)
        {
            ...
            if (member[i].getUserID() == uid)
            {
                member[i].seeBookRentDetail(); //displays the book rent detail of that member
            }
        }
    }
    /*this method updates the book availabilitiy by passing in the book object and the 
    total number of Books*/
    public void updateBookAvailability(Book[] book, int bookMax)
    {
        Scanner input = new Scanner(System.in);
        ...
        do 
        {
            ...
            if (found)
            {
                ...
                book[pos].bookInfo();
                System.out.printf("Please enter the number of available copies for \""+ book[pos].getBookName() + "\": ");
                int num = input2.nextInt();
                book[pos].setBookQuantity(num);
